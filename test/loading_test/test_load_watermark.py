...
def test_fact_watermark_filters_new_rows_and_updates_checkpoint(monkeypatch):
    # ---- FAKES ----
    @dataclass
    class FakeDB:
        ...
        def fetchall(self, sql, params=None):
            # The table name in this test is always fact_sales_order,
            # but we don't even need to branch on it.
            if "data_type" in sql and "is_nullable" in sql:
                return [
                    ("sales_order_id", "integer", "NO"),
                    ("last_updated", "timestamp with time zone", "NO"),
                ]

            return [("sales_order_id",), ("last_updated",)]
    ...
    latest_key = f"{table}/fileC.parquet"
    ...
    fake_s3.parquet[latest_key] = pd.DataFrame(
        [
            {"sales_order_id": 10, "last_updated": "2026-01-06T10:00:00Z"},  # old
            {"sales_order_id": 11, "last_updated": "2026-01-06T10:10:00Z"},  # new
            {"sales_order_id": 12, "last_updated": "2026-01-06T10:12:00Z"},  # new
        ]
    )
    ...
    assert res["watermark"] == "last_updated"

    assert len(fake_db.executemany_calls) == 1
    assert len(fake_db.executemany_calls[0]["params"]) == 2

    # ---- Assert checkpoint updated ----
    written = json.loads(fake_s3.s3.objects[ckpt_key].decode("utf-8"))
    assert written["last_loaded_key"] == latest_key
    assert written["last_loaded_ts"] == "2026-01-06T10:12:00Z"
